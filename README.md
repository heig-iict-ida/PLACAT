...
2. Install the dependencies with `conda install --file requirements.txt`
3. Run `python -m spacy download en_core_web_lg` to download the model used by the neuralcoref module to enable pronouns resolution.
4. Execute `./run_backend.sh` to run PLACAT
5. Either test the application on the simulator in Dialogflow, or use the `qa.py` script to test one question: `python qa.py -q Who is Roger Federer ?`
